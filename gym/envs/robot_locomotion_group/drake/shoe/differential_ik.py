...
class DifferentialIK(LeafSystem):
    ...
    def ForwardKinematicsRpyXyz(self, q):
        X_ee = self.ForwardKinematics(q)
        rpy = RollPitchYaw(X_ee.rotation()).vector()
        xyz = X_ee.translation()
        return np.concatenate((rpy, xyz))

    def CalcPoseError(self, X_WE_desired, q):
        pose = self.ForwardKinematics(q)
        print(pose.translation())
        print(X_WE_desired.translation())
        # err_vec = np.zeros(6)
